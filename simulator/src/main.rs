...
/*
 * This is a simple simulator that simulates network traffic between routers.
 * The number of routers is the tables argument, and the total number of records
 * is the records argument. Since the aggregation phase is influenced by the
 * total number of records involved, the simulator will generate logs as given.
 */
#[tokio::main(flavor = "multi_thread", worker_threads = 10)]
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    let args = Arc::new(Args::parse());

    // Logging (debug logs, not netflow logs) setup
    // All the logs generated by info!(...) will be written to the log file.
    ...
    // Since we use Postgres as the netflow log storage, we need to connect to it.
    ...
    // Create a worker that generates hashes for the logs. We use this hash
    // to verify the integrity of the logs. The aggregation phase checks this.
    let round: i32;
    {
        let num_tables = args.tables;
        let client = Arc::clone(&pg_client);
